...
ORIGINAL_SOURCE_RECORD_TOTAL = 1_840_744
...
def _add_case_reference(doc, conversation_id):
    ...
    _set_run(
        p.add_run(f"Conversation {conversation_id}"), size=9.2, bold=True, color=MUTED
    )
    return p
...
def _add_stage_table(doc, rows, *, split_after=None):
    ...
    for segment_number, segment in enumerate(segments):
        ...
        _set_table_geometry(table, [1600, 900, 3300, 3560])
    p = doc.add_paragraph(
        "Note. Excerpts are shortened; French and Dutch material is translated into English. Turn labels refer to the normalized source transcript."
    )
...
def _add_level_table(doc, campaign):
    ...
    data = [
        (
            "L0",
            "Instrumental task",
            "Task or external artifact",
            f"{counts[0]:,}",
            "The output changes, but no evidenced consumer capability, owned project, or self is extended.",
        ),
        (
            "L1",
            "Capability extension",
            "Situated consumer agency",
            f"{counts[1]:,}",
            "AI augments what this particular consumer can learn, analyze, write, design, or do.",
        ),
        (
            "L2",
            "Project or possession extension",
            "Owned or identity-bearing project",
            f"{counts[2]:,}",
            "AI recursively develops something the consumer compellingly owns, directs, or builds.",
        ),
        (
            "L3",
            "Representational extension",
            "Actual or possible self",
            f"{counts[3]:,}",
            "AI symbolizes the consumer's identity, relationship, aspiration, or voice, and that representation is negotiated.",
        ),
        (
            "L4",
            "Reflexive incorporation",
            "Self-understanding",
            f"{counts[4]:,}",
            "The consumer recognizes, accepts, rejects, or revises an AI-mediated representation so that it enters self-understanding.",
        ),
        (
            "L5",
            "Applied extension",
            "Self-representation as premise",
            f"{counts[5]:,}",
            "The incorporated representation is reused as a premise for a later request, interpretation, choice, project, or practice.",
        ),
    ]
    ...
    for cell, text in zip(
        table.rows[0].cells,
        ["Level", "Construct", "Recursive object", "Cases", "Defining diagnostic"],
    ):
        _shade(cell, BLUE)
...
def _campaign_state(audit_cutoff=None):
    ...
    manual_records = {label: _read_jsonl(path) for label, path in MANUAL_PATHS.items()}
    all_manual_ids = [
    ...
    if len(all_manual_ids) != len(set(all_manual_ids)):
        raise RuntimeError(
            "Canonical manual evidence contains duplicate conversation IDs"
        )
    campaign_manual = [
        record
        for records in manual_records.values()
    ...
    return {
        ...
        "corpus_counts": {
            dataset: int(count) for dataset, count in manifest["counts"].items()
        },
        "source_record_total": ORIGINAL_SOURCE_RECORD_TOTAL
        + sum(
            int(count)
            for count in manifest.get("expansion_source_record_counts", {}).values()
        ),
        "automated_total": len(automated),
        "automated_errors": len(errors),
        "automated_candidates": len(candidate_ids),
        "automated_labels": Counter(
            record["recursive_extension"] for record in automated
        ),
        "automated_levels": Counter(
            int(record["extension_level"]) for record in automated
        ),
        ...
        "manual_levels": Counter(
            int(record["assessment"]["extension_level"]) for record in campaign_manual
        ),
        "clear_levels": Counter(
            int(record["assessment"]["extension_level"]) for record in clear_records
        ),
...
def _validate_translation_record(conversation, translation_record):
    ...
    if any(
        message.get("content", "") and not translated
        for message, translated in zip(conversation["messages"], translations)
    ):
    ...
    if "26b" not in str(translation_record.get("translation_method", "")).lower():
        raise RuntimeError(
            f"Translation for {conversation['id']} was not produced by the required 26B model"
        )
...
def _add_classification_explanation(doc, review):
    ...
    _add_body(
        doc,
        explanation,
        bold_lead=f"This conversation is classified as clear L{level}.",
    )
...
def _add_full_transcript(
    ...
    *,
    ...
    displayed_turns = (
        record["message_turns"] - 2
        if template_noise_cleaned
        else record["message_turns"]
    )
    if include_heading:
        heading = doc.add_heading(
            f"Case {case_number} Conversation {record['id']}", level=1
        )
    ...
    if any(
        re.search(r"[\x00-\x08\x0b\x0c\x0e-\x1f]", message.get("content", ""))
        ...
        for message, translation in zip(record["messages"], translations):
            ...
            for column_index, (cell, text) in enumerate(
                zip(
                    row.cells,
                    [
                        _word_safe_text(message.get("content", "")),
                        _word_safe_text(translation),
                    ],
                )
            ):
                _shade(cell, fill)
    ...
    for source_index, message in enumerate(record["messages"]):
        ...
        for chunk_index, chunk in enumerate(chunks, 1):
            ...
            continued = (
                "" if len(chunks) == 1 else f"  ·  PART {chunk_index} OF {len(chunks)}"
            )
            _set_run(
                bubble.add_run(label + continued),
                size=8.3,
                bold=True,
                color=label_color,
            )
...
def _build_appendix_fragment(record, review, translation_record):
    ...
    return [child.xml for child in body_elements]
...
def _append_case_heading(doc, case_number, conversation_id):
    heading = doc.add_heading(
        f"Case {case_number} Conversation {conversation_id}", level=1
    )
    heading.paragraph_format.page_break_before = case_number > 1
...
def _make_model_figure():
    ...
    def node(rect, node_title, subtitle, face, edge=BLUE):
        draw.rounded_rectangle(
            rect, radius=24, fill=f"#{face}", outline=f"#{edge}", width=4
        )
        x1, y1, x2, y2 = rect
    ...
    def arrow(start, end, label=None):
        ...
        if label:
            ...
            draw.rounded_rectangle(
                (mx - tw / 2 - 7, my - th / 2 - 7, mx + tw / 2 + 7, my + th / 2 + 7),
                radius=5,
                fill="white",
            )
    ...
    centered(
        "A gated, recursive model of extending with AI", (1080, 72), title_font, INK
    )
    centered(
        "Levels locate the recursive object; gates mark qualitative changes in its relation to the consumer",
        (1080, 132),
        regular,
        MUTED,
    )
    centered(
        "At every nonzero level: externalization  →  AI transformation  →  user uptake  →  recursive re-entry",
        (1080, 208),
        regular,
        DARK_BLUE,
    )
    ...
    centered(
        "applications and their consequences become new externalizations",
        (1080, 1000),
        regular,
        DARK_BLUE,
    )
...
def _make_process_figure(campaign):
    ...
    def box(rect, title, detail, *, fill=LIGHTER_BLUE, edge=BLUE, heading_font=bold):
        draw.rounded_rectangle(
            rect, radius=24, fill=f"#{fill}", outline=f"#{edge}", width=4
        )
        x1, y1, x2, y2 = rect
    ...
    centered(
        "Corpus Screening and Manual Adjudication", (80, 35, 1720, 130), title_font
    )
    centered(
        f"Campaign state  {_format_capture_date(campaign['captured_at'])}",
        (80, 120, 1720, 190),
        regular,
        MUTED,
    )
    ...
    box(
        (main_x1, rows[0][0], main_x2, rows[0][1]),
        "Source Conversations Collected",
        f"n = {campaign['source_record_total']:,}\nEight pinned source releases",
    )
    box(
        (main_x1, rows[1][0], main_x2, rows[1][1]),
        "Long Conversation Corpus",
        f"n = {campaign['corpus_total']:,}\nAt least 10 user and 10 assistant turns",
    )
    box(
        (main_x1, rows[2][0], main_x2, rows[2][1]),
        "Automated V5 Screening Completed",
        f"n = {campaign['automated_total']:,}\nGemma 4 26B structured retrieval",
    )
    box(
        (main_x1, rows[3][0], main_x2, rows[3][1]),
        "Clear or Potential Candidates Retrieved",
        f"n = {campaign['automated_candidates']:,}\nAdvanced to complete transcript review",
    )
    box(
        (main_x1, rows[4][0], main_x2, rows[4][1]),
        "Manually Adjudicated Candidates",
        f"n = {campaign['manual_total']:,}\nClear {campaign['manual_labels']['clear']:,}   Potential {campaign['manual_labels']['potential']:,}   None {campaign['manual_labels']['none']:,}",
        fill=LIGHT_GOLD,
        edge=GOLD,
    )
    ...
    side = [
        (
            ...
            side_x2,
            810,
            "Below 10 by 10 or Duplicate",
            f"n = {campaign['source_record_total'] - campaign['corpus_total']:,}",
        ),
        (
            side_x1,
            970,
            ...
            1170,
            "Not Yet Screened",
            f"n = {campaign['corpus_total'] - campaign['automated_total']:,}",
        ),
        (
            side_x1,
            1330,
            ...
            1530,
            "Not Retrieved",
            f"n = {campaign['automated_total'] - campaign['automated_candidates']:,}",
        ),
        (
            side_x1,
            1700,
    ...
    for upper, lower in zip(rows, rows[1:]):
        arrow(
            ((main_x1 + main_x2) // 2, upper[1]), ((main_x1 + main_x2) // 2, lower[0])
        )
    for y in (710, 1070, 1430, 1810):
        draw.line([(main_x2, y), (side_x1, y)], fill=f"#{MUTED}", width=4)
        draw.polygon(
            [(side_x1, y), (side_x1 - 26, y - 14), (side_x1 - 26, y + 14)],
            fill=f"#{MUTED}",
        )
...
def build_draft(campaign):
    ...
    _set_run(
        kicker.add_run("INTERPRETIVE MANUSCRIPT DRAFT"), size=10, bold=True, color=GOLD
    )
    title = doc.add_paragraph(style="Title")
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    title.paragraph_format.space_after = Pt(10)
    _set_run(
        title.add_run("AI Mediated Recursive Self Extension"),
        size=27,
        bold=True,
        color=BLACK,
        font="Aptos Display",
    ...
    _set_run(
        subtitle.add_run("From representation to incorporation and application"),
        size=15,
        color=DARK_BLUE,
        font="Aptos Display",
    )
    ...
    _add_callout(
        doc,
        "Central finding",
        "The decisive process is representational rather than behavioral: consumers first incorporate an AI-mediated representation into self-understanding and then apply it as a premise for what comes next. The application may concern a subsequent request, interpretation, choice, project, or practice; it need not be verified outside the conversation.",
    )
    doc.add_page_break()
    ...
    _add_body(
        doc,
        f"Generative AI does more than deliver recommendations or help consumers produce artifacts. Across extended conversations, consumers can externalize self-relevant material, receive an AI transformation of it, negotiate or accept that representation, and then reuse it as a premise for what follows. We conceptualize this process as AI-mediated recursive self-extension. Using an iterative abductive mixed-method design, we construct a corpus of {campaign['corpus_total']:,} public conversations containing at least ten user and ten assistant messages. At the present campaign checkpoint, computational candidate retrieval covers {campaign['automated_total']:,} conversations and human-led qualitative adjudication covers {campaign['manual_total']:,} retrieval-enriched candidate records. Boundary examples and revelatory cases show how the object carried forward can move from an external task toward capability, possession, self-representation, self-understanding, and application. From these findings, we develop a process model of increasingly self-proximal recursive extension. The main theoretical contribution is to locate consequential extension neither in AI use nor in verified offline action, but in the chronological process by which an AI-mediated representation enters self-understanding and becomes a premise for later requests, interpretations, choices, projects, or practices.",
    )
    ...
    _set_run(
        keywords.add_run(
            "extended self; generative AI; consumer identity; recursion; human–AI interaction; mixed methods"
        ),
        size=9.5,
        italic=True,
        color=MUTED,
    ...
    _add_body(
        doc,
        "The gap is therefore not a lack of work on AI, identity, or digital extension. It lies at their intersection and in their temporal organization. Existing work commonly treats the AI system as a service provider, interface, social partner, capability, or research collaborator and examines acceptance, experience, relationship, or performance outcomes. Extended-self work commonly explains how people invest identity in objects and digital possessions. What remains undertheorized is a chronological consumer process in which the object of recursion progressively moves closer to the self: from task, to capability or project, to representation, incorporation, and application.",
    )
    _add_callout(
        doc,
        "Research question",
        "How do extended consumer–AI interactions recursively transform self-relevant material into representations that consumers incorporate and apply, and what qualitative boundaries distinguish this process from ordinary iterative AI use?",
    )
    ...
    corpus_counts = campaign["corpus_counts"]
    _add_body(
        doc,
        f"We normalized public conversations from eight pinned releases: WildChat-1M, LMSYS-Chat-1M, ThoughtTrace, the preview release of ChatGPT-RealUser-2.2M, ShareGPT-X, PRISM Alignment, ShareChat, and WildChat-4.8M. To make chronological uptake and re-entry observable, the analytic corpus retains conversations with at least ten user and ten assistant messages and removes exact transcript duplicates. This produces {campaign['corpus_total']:,} conversations: {corpus_counts['WildChat-1M']:,} from WildChat-1M, {corpus_counts['LMSYS-Chat-1M']:,} from LMSYS-Chat-1M, {corpus_counts['ThoughtTrace']:,} from ThoughtTrace, {corpus_counts['ChatGPT-RealUser-2.2M-preview']:,} from the RealUser preview, {corpus_counts['ShareGPT-X']:,} from ShareGPT-X, {corpus_counts['PRISM-alignment']:,} from PRISM, {corpus_counts['ShareChat']:,} from ShareChat, and {corpus_counts['WildChat-4.8M']:,} non-overlapping records from WildChat-4.8M. PRISM retains only participant-selected model replies. The full RealUser dataset was not available and no preview-based extrapolation is treated as observed data.",
    )

    doc.add_heading("Machine-assisted candidate retrieval", level=2)
    _add_body(
        doc,
        f"An iterative calibration process developed five successive audit instruments. The current v5 audit uses a locally hosted 26-billion-parameter Gemma-family model with schema-constrained compact JSON, temperature 0, a fixed seed, four workers, and four 131,072-token contexts. Transcripts are conservatively chunked at 115,000 Unicode characters to leave room for the rubric and output. The first chunk is assessed first; later chunks are reviewed sequentially only when the first is positive under the retrieval predicate, with the cumulative assessment confirmed, rejected, or revised at each step. At the campaign-state capture, v5 had completed {campaign['automated_total']:,} of {campaign['corpus_total']:,} conversations ({campaign['automated_total'] / campaign['corpus_total']:.1%}) with {campaign['automated_errors']:,} records in the error ledger. Its provisional labels were {campaign['automated_labels']['clear']:,} clear, {campaign['automated_labels']['potential']:,} potential, and {campaign['automated_labels']['none']:,} none; these labels function only as retrieval decisions before human adjudication.",
    )
    _add_body(
        doc,
        "The automated prompt deliberately protects recall for potential cases while reserving 'clear' for complete evidence of the focal recursive process. It assigns consumption relevance broadly, distinguishes artifact recursion from possible self-extension, identifies the strongest coherent topic segment, proposes a provisional category, and emits stage evidence. This stage is best understood as theoretically informed retrieval and structuring, not automated qualitative adjudication.",
    )
    ...
    process_shape = process_paragraph.add_run().add_picture(
        str(PROCESS_FIG_PATH), width=Inches(6.35)
    )
    _set_image_alt(
        process_shape,
    ...
    process_caption = doc.add_paragraph(
        "Figure 1. Corpus construction, automated screening, candidate retrieval, and manual adjudication at the current campaign checkpoint."
    )
    ...
    _add_body(
        doc,
        "The strongest observed pattern adds another recursive step. An AI-mediated representation is not only negotiated or incorporated; it becomes a premise for a later request, interpretation, choice, project, or practice. Three revelatory cases show different forms of this application.",
    )
    ...
    _set_run(
        case_id.add_run(
            "Conversation 7f456b31db9f8c5815da09e3  |  Housing, mobility, belonging"
        ),
        size=9.5,
        bold=True,
        color=MUTED,
    ...
    _add_stage_table(
        ...
        [
            (
                "Externalization",
                "U4–U5",
                "The user plans to leave provincial life and asks where in Manila life is more accepting for LGBTQ+ people.",
                "A relocation problem becomes anchored in independence, identity safety, and belonging.",
            ),
            (
                "AI transformation",
                "A5",
                "AI maps that possible life onto Malate, Quezon City, Poblacion, and BGC, attaching community and lifestyle meanings to place.",
                "The city is represented as a portfolio of material environments in which the possible self might be lived.",
            ),
            (
                "Uptake",
                "U6–U8",
                "The user retains the acceptance criterion but adds southern geography, same-generation peers, mobility, amenities, and a demand for neighborhood specificity.",
                "Rather than merely requesting more, the user revises the AI's spatial representation through increasingly personal constraints.",
            ),
            (
                "Recursive re-entry",
                "U9; U11",
                "The user asks which part of AI-recommended Makati is affordable at a stated income and later how to explain the desired city life to parents.",
                "The AI-configured future re-enters budgeting, housing search, and family communication as a consequential premise.",
            ),
        ],
    )
    ...
    _add_body(
        doc,
        "The extended object is neither a discrete possession nor a textual self-description. It is an assemblage of neighborhood, transport, affordability, consumption venues, peer community, and family negotiation. AI helps make the imagined urban self actionable by translating it into a material ecology. This case therefore expands the extended-self lens from possession attachment to infrastructural configuration: the self is extended through a prospective arrangement of market and civic resources.",
    )
    _add_callout(
        doc,
        "Interpretive boundary",
        "The decisive threshold is crossed when the user applies the AI-mediated possible self to affordability, neighborhood choice, and family communication. Whether relocation later occurred is not constitutive of the process; it would only provide additional contextual evidence about the application's consequences.",
    )

    doc.add_heading(
        "Incorporating and applying a relational self-representation", level=3
    )
    case_id = doc.add_paragraph()
    case_id.paragraph_format.space_after = Pt(8)
    _set_run(
        case_id.add_run(
            "Conversation 118ea59bc07477688fe6e093  |  Relational incorporation and application"
        ),
        size=9.5,
        bold=True,
        color=MUTED,
    ...
    _add_stage_table(
        ...
        [
            (
                "Externalization",
                "U3",
                "The user says they like a woman but cannot tell her what they feel and asks for advice.",
                "A vulnerable relational aspiration and uncertainty are made available for AI mediation.",
            ),
            (
                "AI transformation",
                "A3",
                "AI frames disclosure as honest communication bounded by listening, non-coercion, and respect for any response.",
                "The problem is transformed from obtaining reciprocation into conducting oneself respectfully under uncertainty.",
            ),
            (
                "Uptake",
                "U4–U8",
                "The user agrees to try and starts configuring a restaurant surprise; the advice becomes part of planning a social encounter.",
                "Acceptance is explicit and elaborative, not a generic request for additional text.",
            ),
            (
                "Recursive re-entry",
                "U18",
                "The user reports organizing a dinner after the AI's advice and requests a concise statement; AI supplies a non-pressuring disclosure script.",
                "Reported offline action returns to the dialogue, where it becomes the premise for another AI-configured step.",
            ),
        ],
        split_after=1,
    )
    doc.add_heading(
        "Mechanism: applying an incorporated relational orientation", level=3
    )
    _add_body(
        doc,
        "The extension is not that AI speaks on the user's behalf. AI represents a possible way of being in the relationship—honest, respectful, and non-coercive—and the user accepts that orientation before applying it to the setting and wording of disclosure. The reported dinner makes this application especially visible, but the theoretical process is already present in the recursive reuse of the representation: lived uncertainty becomes an AI-mediated relational orientation, which becomes a premise for subsequent planning and expression.",
    )
    _add_callout(
        doc,
        "Interpretive boundary",
        "The analytically central evidence is the user's acceptance and application of the AI's representation of respectful relational conduct. The reported dinner strengthens the temporal chain, but the process does not depend on verifying its outcome or on showing that a relationship formed.",
    )
    ...
    _set_run(
        case_id.add_run(
            "Conversation c266abbc2a53871b6c894722  |  Self-modeling and marketplace navigation"
        ),
        size=9.5,
        bold=True,
        color=MUTED,
    ...
    _add_stage_table(
        ...
        [
            (
                "Externalization",
                "U2",
                "The user narrates design, coordination, prioritization, client communication, technical learning, and five years of responsibility, then asks what skills they possess.",
                "A biographical work account is offered as raw material for an externally generated self-model.",
            ),
            (
                "AI transformation",
                "A2–A7",
                "AI converts the narrative into named skills, qualities, and pitfalls, eventually summarizing the user as a designer, project manager, communicator, and problem solver.",
                "Diffuse experience becomes a portable categorical representation of who the user is and what they can offer.",
            ),
            (
                "Uptake",
                "U3–U8",
                "The user repeatedly requests more skills, qualities, pitfalls, and summaries, thereby elaborating and consolidating the profile.",
                "The user treats the model as revisable self-knowledge rather than merely résumé prose.",
            ),
            (
                "Recursive re-entry",
                "U9–U10",
                "The user asks which industries to approach for new clients and which roles fit if UI/UX is removed from consideration.",
                "The AI-produced profile becomes an instrument for navigating clients, sectors, and alternative marketplace selves.",
            ),
        ],
    )
    ...
    _set_image_alt(
        shape,
        "Gated recursive extension model",
        "Diagram showing L0 task recursion branching into L1 capability and L2 project pathways, converging at L3 representation, progressing through L4 incorporation to L5 application, with applications and their consequences feeding back into renewed externalization.",
    )
    caption = doc.add_paragraph(
        "Figure 2. Gated recursive progression from task recursion to incorporation and application of an AI-mediated self-representation."
    )
    ...
    propositions = [
        (
            "P1 — Recursive-object proposition.",
            " The level of AI extension depends on what recursively re-enters the interaction: an artifact, capability, owned project, representation, self-understanding, or action premise.",
        ),
        (
            "P2 — Gated-deepening proposition.",
            " Repetition alone does not deepen extension. Upward movement requires a qualitative gate: anchoring, ownership, self-reference, reflexive incorporation, or application.",
        ),
        (
            "P3 — Application proposition.",
            " AI-mediated self-representations become more consequential when consumers reuse them as premises across requests, interpretations, choices, projects, or practices; movement outside the conversation is one possible form, not the defining one.",
        ),
        (
            "P4 — Recursive-risk proposition.",
            " The same application that enables extension can stabilize weakly grounded AI inferences, allowing speculative self-attributes to shape subsequent interpretation and opportunity perception.",
        ),
        (
            "P5 — Re-entry proposition.",
            " Applications and their consequences generate new material that can be externalized again, making AI extension cyclical rather than terminal.",
        ),
    ]
    ...
    _add_body(
        doc,
        "These cases suggest that the AI-extended self is not produced by personalization alone. It emerges when consumers permit, contest, revise, incorporate, and reuse AI transformations of self-relevant material. At lower levels, AI changes outputs, capabilities, and owned projects. At the representational threshold, the recursive object becomes the consumer or a possible self. Reflexive incorporation then makes that representation part of self-understanding; application gives it recursive force by making it a premise for what follows. The progression is therefore neither technological nor automatic. It is an accomplishment of representational uptake and reuse.",
    )

    doc.add_heading("Contribution to extended-self theory", level=2)
    _add_body(
        doc,
        "The first contribution is to shift the analytic question from what object is attached to the self to what object recursively returns. Belk's (1988) account explains how possessions extend identity through control, creation, and incorporation. Our model retains capability, ownership, creation, and identity-bearing projects, but shows that conversational AI can produce an intermediate symbolic object—a representation of the actual or possible consumer—that is explicitly available for negotiation. Extension deepens when the consumer incorporates and applies that representation. The distinctive extension object is therefore not necessarily the AI system or its output as a possession; it can be the AI-mediated formulation that becomes part of how the consumer understands and organizes the self.",
    )
    _add_body(
        doc,
    ...
    _add_body(
        doc,
        "The digital extended self is dematerialized, shareable, co-constructed, and distributed (Belk 2013). AI-mediated recursive self-extension specifies a new form of co-construction: dialogical and temporally cumulative representation. The consumer supplies partial self-material, the model transforms it, and the consumer's uptake selects which formulation gains continuity. This process can make self-representations portable across domains, as when a retrospective capability account becomes a map of sectors and roles, or a desired way of living becomes a housing and mobility configuration.",
    )
    _add_body(
        doc,
        "This portability also complicates control. Digital self-extension often raises questions about access, ownership, audience, and memory. Generative AI adds inferential momentum: an attribute weakly grounded in the original account can be restated, elaborated, and then treated as input. The same recursive structure that enables the consumer can stabilize error. Contestation and rejection are therefore not failures of extension but core mechanisms through which consumers govern an emergent representation.",
    )
    ...
    future_directions = [
        (
            ...
            " When do AI-mediated representations survive session boundaries, transfer across services, or become part of durable identity narratives?",
        ),
        (
            "Accuracy, uncertainty, and contestation.",
            " How do confidence cues, evidence provenance, and invitations to disagree affect whether weak inferences are incorporated or rejected?",
        ),
        (
            "Power and marketplace sorting.",
            " When do recursively applied profiles expand consumer opportunity, and when do they produce path dependence, exclusion, or discriminatory market segmentation?",
        ),
        (
            "Relational and collective selves.",
            " How does recursive extension operate for couples, families, communities, fan cultures, and ventures when ownership and representation are shared or contested?",
        ),
        (
            "Material consequences without an offline threshold.",
            " Longitudinal research can study when conversational application changes spending, mobility, relationships, learning, or work while keeping such consequences analytically distinct from the L5 definition.",
        ),
        (
            "Model and interface variation.",
            " Memory, anthropomorphic framing, initiative, grounding, and proactive recommendation may alter the gates between representation, incorporation, and application.",
        ),
        (
            "L6 as an outlook rather than an observed coding level.",
            " Future systems may coordinate multiple services, persist representations autonomously, or negotiate on consumers' behalf. Such delegated or infrastructural extension should remain a prospective construct until evidence supports a defensible boundary beyond L5.",
        ),
    ]
    ...
    props.comments = (
        "Generated from manually adjudicated public consumer–AI conversation records."
    )
...
def build_appendix(
    ...
    *,
    ...
    conversations = (
        _load_conversations(review_by_id)
    ...
    if not rebuild_cache and _appendix_output_is_current(
        level, output, document_fingerprint
    ):
    ...
    for case_number, review in enumerate(reviews, 1):
        ...
        if translation_record is not None:
            _validate_translation_record(
                conversations[conversation_id], translation_record
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate the live paper draft and evidence appendices."
    )
    parser.add_argument("--draft-only", action="store_true")
    parser.add_argument(
        "--appendix-level", type=int, choices=(3, 4, 5), action="append"
    )
    parser.add_argument(
        "--audit-cutoff",
        type=int,
    ...
    parser.add_argument(
        "--translation-workers", type=int, default=DEFAULT_TRANSLATION_WORKERS
    )
    ...
    if not args.draft_only:
        ...
        reviews_by_level = {
            level: _appendix_reviews(level, campaign) for level in levels
        }
